...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        # file_client The function of is to read the file according to the given target location , Return binary 
        img_bytes = self.file_client.get(filename)
        # mmcv.imfrombytes, Is to read binary image files 
        ...
        # Add the read picture to results In the dictionary 
        results['img'] = img
...
class RandomGrayscale(object):
    ...
    def __init__(self, gray_prob=0.1, file_client_args=dict(backend='disk',)) :
...
class RandomFlip(object):
    ...
    def __init__(self, flip_prob=0.5, direction='horizontal', file_client_args=dict(backend='disk'),):
...
class Normalize(object):
    ...
    def __init__(self, mean, std, to_rgb=True, file_client_args=dict(backend='disk'),):
