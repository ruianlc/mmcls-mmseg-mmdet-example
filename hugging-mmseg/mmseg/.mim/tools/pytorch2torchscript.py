...
def pytorch2libtorch(model,
                     ...
                     output_file='tmp.pt',
                     verify=False):
    """Export Pytorch backbone to TorchScript backbone and verify the outputs are
    same between Pytorch and TorchScript.

    Args:
        model (nn.Module): Pytorch backbone we want to export.
        input_shape (tuple): Use this input shape to construct
            the corresponding dummy input and execute the backbone.
        show (bool): Whether print the computation graph. Default: False.
        output_file (string): The path to where we store the
            output TorchScript backbone. Default: `tmp.pt`.
    ...
    print('Successfully exported TorchScript backbone: {}'.format(output_file))
...
def parse_args():
    ...
    parser.add_argument(
        '--verify', action='store_true', help='verify the TorchScript backbone')
...
if __name__ == '__main__':
    ...
    # convert the PyTorch backbone to LibTorch backbone
    pytorch2libtorch(
        segmentor,
        input_shape,
        show=args.show,
