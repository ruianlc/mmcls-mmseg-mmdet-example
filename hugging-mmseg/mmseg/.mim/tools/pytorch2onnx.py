...
def pytorch2onnx(model,
    ...
    """Export Pytorch backbone to ONNX backbone and verify the outputs are same
    between Pytorch and ONNX.

    Args:
        model (nn.Module): Pytorch backbone we want to export.
        mm_inputs (dict): Contain the input tensors and img_metas information.
        opset_version (int): The onnx op version. Default: 11.
        show (bool): Whether print the computation graph. Default: False.
        output_file (string): The path to where we store the output ONNX backbone.
            Default: `tmp.onnx`.
    ...
    with torch.no_grad():
        ...
        print(f'Successfully exported ONNX backbone: {output_file}')
...
def parse_args():
    ...
    parser.add_argument(
        '--verify', action='store_true', help='verify the onnx backbone')
...
if __name__ == '__main__':
    ...
    # build the backbone and load checkpoint
    ...
    msg += blue_text + 'Welcome to use the unified backbone deployment toolbox '
