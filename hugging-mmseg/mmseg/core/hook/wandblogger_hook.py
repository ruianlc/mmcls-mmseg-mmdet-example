...
class MMSegWandbHook(WandbLoggerHook):
    ...
    information - saves backbone checkpoints as W&B Artifact, and
    logs backbone prediction as interactive W&B Tables.
    ...
    - Checkpointing: If `log_checkpoint` is True, the checkpoint saved at
      ...
      to learn more about backbone versioning with W&B Artifacts.
    ...
    - Evaluation: At every evaluation interval, the `MMSegWandbHook` logs the
      backbone prediction as interactive W&B Tables. The number of samples
    ...
    Args:
        ...
        log_checkpoint (bool): Save the checkpoint at every checkpoint interval
            as W&B Artifacts. Use this for backbone versioning where each version
            is a checkpoint.
            Default: False
    ...
    def _log_ckpt_as_artifact(self, model_path, aliases, metadata=None):
        """Log backbone checkpoint as  W&B Artifact.
        ...
        model_artifact = self.wandb.Artifact(
            f'run_{self.wandb.run.id}_model', type='backbone', metadata=metadata)
        model_artifact.add_file(model_path)
        self.wandb.log_artifact(model_artifact, aliases=aliases)

    def _get_eval_results(self):
        """Get backbone evaluation results."""
    ...
    def _init_pred_table(self):
        """Initialize the W&B Tables for backbone evaluation."""
    ...
    def _log_eval_table(self, iter):
        """Log the W&B Tables for backbone evaluation.
