...
class BEiT(BaseModule):
    ...
    def _geometric_sequence_interpolation(self, src_size, dst_size, sequence,
        ...
        Args:
            src_size (int): Pos_embedding size in pre-trained backbone.
            dst_size (int): Pos_embedding size in the current backbone.
            sequence (tensor): The relative position bias of the pretrain
                backbone after removing the extra tokens.
    ...
    def resize_rel_pos_embed(self, checkpoint):
        ...
        Args:
            checkpoint (dict): Key and value of the pretrain backbone.
        Returns:
            state_dict (dict): Interpolate the relative pos_embed weights
                in the pre-train backbone to the current backbone size.
        """
